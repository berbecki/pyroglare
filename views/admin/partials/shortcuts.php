...
<nav id="shortcuts" class="clearfix">
	<ul>
		<?php if ( ! empty($module_details['sections'][$active_section]['shortcuts'])): ?>
			<?php foreach ($module_details['sections'][$active_section]['shortcuts'] as $shortcut):
				...
				unset($shortcut['name']);
				unset($shortcut['uri']); ?>
			<li><a class="btn add" <?php foreach ($shortcut as $attr => $value) echo $attr.'="'.$value.'"'; echo 'href="' . site_url($uri) . '"><i class="icon-plus-sign"></i> ' . lang($name) . '</a>'; ?></li>
			<?php endforeach; ?>
		<?php endif; ?>
